...
template sudoku() {
    ...
    component puzzle_RangeProof[9][9];
    component solution_RangeProof[9][9];


    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {

            puzzle_RangeProof[i][j] = RangeProof(32);
            solution_RangeProof[i][j] = RangeProof(32);

            puzzle_RangeProof[i][j].in <== puzzle[i][j];
            puzzle_RangeProof[i][j].range[0] <== 0;
            puzzle_RangeProof[i][j].range[1] <== 9;

            solution_RangeProof[i][j].in <== solution[i][j];
            solution_RangeProof[i][j].range[0] <== 0;
            solution_RangeProof[i][j].range[1] <== 9;

            assert(puzzle_RangeProof[i][j].out == 1);
            assert(solution_RangeProof[i][j].out == 1);
